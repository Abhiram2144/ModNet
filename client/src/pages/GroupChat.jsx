...
export default function GroupChat() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (file) {
        ...
        const { data: publicUrlData } = supabase.storage.from("attachments").getPublicUrl(uploadData.path);
  ...
  if (allowed === null) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-100 text-gray-500">Checking access…</div>
    );
  }
  ...
  if (!allowed) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-gray-100 px-6 text-center text-gray-900">
        ...
        <p className="mb-6 max-w-sm text-gray-600">You don't have access to this group's chat.</p>
        <button onClick={() => navigate("/discover")} className="rounded-full bg-blue-600 px-5 py-2 text-white transition hover:bg-blue-500">
          Back to Discover
        </button>
      </div>
    );
  ...
  return (
    <div className="font-inter flex flex-col text-gray-900 h-[100dvh] max-h-[100dvh] overflow-hidden">
      ...
      <div className="flex items-center px-4 py-3 shadow-sm flex-shrink-0 h-16 min-h-16 max-h-16 bg-white z-10">
        <button onClick={() => navigate(-1)} className="mr-3 text-gray-500 hover:cursor-pointer hover:text-gray-700">
          <ArrowLeft size={22} />
        </button>
        <h1 className="truncate text-lg font-semibold">{channelInfo?.name || "Group"}</h1>
      </div>
                ...
                <div className={`max-w-[70%] rounded-2xl px-4 py-2 shadow-sm ${mine ? "rounded-br-none bg-blue-600 text-white" : "rounded-bl-none bg-gray-200 text-gray-800"}`}>
                  {parent && (
                    <div className={`mb-2 rounded-lg border ${mine ? "border-blue-400/40 bg-blue-500/20" : "border-gray-300 bg-white/60"} px-3 py-2`}>
                      ...
                      {parent.content && <div className="line-clamp-2 text-xs text-gray-700">{parent.content}</div>}
                      {!parent.content && parent.attachment_name && (
                        <div className="text-xs text-gray-700">Attachment: {parent.attachment_name}</div>
                      )}
                    </div>
                  )}
                  {!mine && <span className="mb-1 block text-xs text-gray-500">{msg.students?.displayname || "User"}</span>}
                  {msg.content && <p className="text-sm font-medium">{msg.content}</p>}
                  {msg.attachment_url && (
                    <a href={msg.attachment_url} target="_blank" rel="noreferrer" className={`mt-1 block text-xs underline ${mine ? "text-blue-100" : "text-blue-600"}`}>
                      {msg.attachment_name || "View Attachment"}
                    </a>
                  )}
                  <span className={`mt-1 block text-[10px] ${mine ? "text-blue-100" : "text-gray-500"} text-right`}>
                    {new Date(msg.created_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: true, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone })}
                  </span>
          ...
          {replyTarget && (
            <div className="flex w-full items-start justify-between rounded-lg border border-gray-300 bg-white px-3 py-2">
              <div className="mr-3 min-w-0">
                ...
                {replyTarget.content && <div className="line-clamp-2 text-xs text-gray-600 break-words">{replyTarget.content}</div>}
            ...
            <div className="flex w-full flex-col space-y-1">
              <div className="flex items-center justify-between w-full rounded-lg bg-white px-3 py-2 border border-gray-300">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="text-sm font-medium text-gray-800 truncate max-w-[60vw]">{file.name}</div>
                </div>
